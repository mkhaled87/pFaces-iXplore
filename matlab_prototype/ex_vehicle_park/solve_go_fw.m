...
options.TargetSet_lb = [8.75 2.25 -0.25 -0.25 -0.25];
options.TargetSet_ub = [9.25 2.75 +0.25 +0.25 +0.25];
options.SpaceDiameter = 12;
options.TargetSetCenter = [9 2.5 0 0 0 0 0];
options.initial_state = [1 1 0 0 0 0 0];
options.Ts = 0.100;


[bestTraces, bestQualities, finished_slots] = ...
    solveBranchAndBound(@vehicleCheckSpecs_fw, options);
...
elseif(bestQualities(finished_slots) == 1)
    ...
    [how_good, trajectory] = vehicleCheckSpecs_fw(bestTrace, options);
